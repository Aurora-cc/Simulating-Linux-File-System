...
void set_cur_path(dir dirCurDir)
{
	...
	if (dirCurDir.inodeNum != 0)
	{
		...
		virDisk.seekg(inodeTable[dirCurDir.nSubInode[1]].diskAddr, ios::beg);
	...
	else
	{
		...
		ssStream << curPath << "/" << inodeTable[dirTemp.inodeNum].name;
		ssStream >> curPath;
		ssStream.clear();
	}
}
...
bool path_to_dir(const char* strPath, size_t nPathLen, size_t& nPos, char* strDirName, dir& dirTemp)
{
	...
	while (get_dir_name(strPath, nPathLen, nPos, strDirName))
	{
		...
		{
			//查找是否存在同名子目录
			if (strcmp(strDirName, inodeTable[dirTemp.nSubInode[i]].name) == 0 &&
				inodeTable[dirTemp.nSubInode[i]].type == TYPE_DIR)
			{
				virDisk.seekg(inodeTable[dirTemp.nSubInode[i]].diskAddr, ios::beg);
...
bool analyse_path(const char* strPath, size_t nPathLen, dir& dirTemp, char* strFileName)
{
	...
	const char* strDiv = strrchr(strPath, '/'); //从尾到头搜索，返回第一处字符'/'的位置
	if (strDiv)
	{
		...
		virDisk.open("virtual_disk.bin", ios::in | ios::binary);
		if (!virDisk.is_open()) error(FATAL_READ);
		virDisk.seekg(inodeTable[0].diskAddr, ios::beg);
		virDisk.read(reinterpret_cast<char*>(&dirTemp), sizeof(dir));
		...
		{
			//查找
			virDisk.seekg(inodeTable[dirCurPath.nSubInode[1]].diskAddr, ios::beg);
			virDisk.read(reinterpret_cast<char*>(&dirTemp), sizeof(dir));
			nPos++;
			//能找到路径对应的目录，则 OK；否则返回失败
			if (path_to_dir(strNewPath, nLen, nPos, strDirName, dirTemp))
...
bool analyse_path(const char* strPath, size_t nPathLen, dir& dirTemp)
{
	...
	if (strcmp(strPath, "..") == 0)	//上层目录
	{
		virDisk.open("virtual_disk.bin", ios::in | ios::binary);
		if (!virDisk.is_open()) error(FATAL_READ);
		virDisk.seekg(inodeTable[dirTemp.nSubInode[1]].diskAddr, ios::beg);
		virDisk.read(reinterpret_cast<char*>(&dirTemp), sizeof(dir));
		virDisk.close();
		return true;
	}
